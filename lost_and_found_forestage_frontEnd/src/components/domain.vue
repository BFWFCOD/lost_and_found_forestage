  ...
  <div v-loading="">
    ...
    >
      <el-card
        ...
        v-for="(pickthing, index) in pickthingsDataList"
        :key="'pick' + index"
        ...
        <div>
          ...
          >
            <el-button
              ...
              @click="lookInfoClick(pickthing, 0)"
              >查看发布者信息
            </el-button>
          </div>
          <div
      ...
      <el-card
        style="width: 330px; margin-top: 10px"
        v-for="(lostthing, index) in lostthingsDataList"
        :key="'lost' + index"
        v-loading="cardloading[index]"
        shadow="always"
      >
        ...
        <div>
          ...
          >
            <el-button
              ...
              size="small"
              style="margin-left: 3px"
              @click="lookInfoClick(lostthing, 1)"
              >查看发布者信息
            </el-button>
          </div>
          <div
  ...
  methods: {
    ...
    loadUserList() {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          this.pickthingsDataList = data.page.list;
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          this.lostthingsDataList = data.page.list;
          this.totalPage = data.page.totalCount;
          if (this.pageSize >= this.totalPage) {
            this.noMore = true;
          }
    ...
    lookInfoClick(user, index) {
      this.userinfovisible = true;
      this.clickedUserData = user;
      console.log("父组件：clickedUserData:", this.clickedUserData);
      this.$nextTick(() => {
        this.$refs.lookUserInfo.init(index);
      });
    }
  }, // 方法集合
